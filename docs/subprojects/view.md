...
Branch `view/mission-file`. Row 5 of entry 045's road, and **it waits on Content**, which is
writing the file on `content/mission-file`. Until it exists there is nothing to load, so read
this, check whether the file has landed, and if it has not, the honest answer is that this
territory has nothing to build until the greybox.

**What it will replace.** `SandboxScenario` holds four things a mission file holds: which map,
who starts where with what facing, where our side may leave from, and the bar we may not be
noticed above. Entry 038 counted three copies of the waystation mission and entry 046 added the
fourth knowingly — the map header carries it in prose, the harness in `content/` deploys it, and
this file does both. **When the file lands, this scenario becomes a deletion**, which is what
gathering it in one place was for. The compound has no mission and does not want one; it exists
so that a capture from before the waystation can still be reproduced.

**What to watch for.** A mission file names an objective kind, and the rules have one —
`Withdrawal`. The other five in the mission book are Core's and are not built, so the loader has
to fail on a kind it does not know in a way that says which kind, rather than dropping the
objective and leaving a sandbox that quietly cannot be won.
...
Entry 046 is the reasoning; this is the shape.
...
- **Walls and ground take their look from their figures, never from their id.** Entry 038 asked
  for the decision and entry 046 made it. All six built-in wall profiles come out at the colour
...
- **A switch over somebody else's enum wants its default to be a sentence, not a guess.**
  ...
  AI threw brought the whole frame down. Entry 046. There are two more switches over Core enums
...
carries the outcome (entry 040) and a handed-out window is answerable by hand (entry 046).
