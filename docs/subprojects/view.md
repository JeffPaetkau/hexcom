...
peak for a hostile stopped at a window — which is brief two's ground and entry 088's finding.
...
Brief five. `../decisions.md` entry 088 is the reasoning, the measurement and the finding; this is the
shape.
...
- ~~**Whether an unfound hostile should hold a slot in the turn order at all.**~~ Answered by
  `../decisions.md` entry 064, and the answer is no. Built by brief five, entry 088.
...
- ~~**Whether the top block should describe a hostile who is up.**~~ No — it read the enemy's
  contact file as a number. `SandboxFrame.Withheld`, brief five, entry 088.
