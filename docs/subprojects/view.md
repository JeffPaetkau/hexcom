...
The action bar landed — *What landed on `view/action-bar`* below, and `../decisions.md` entry 095.
...
- **Keys.** Outside a window the number keys are the bar's — `1`–`3` the fire modes cheapest first,
  ...
  nothing else**, and `Backspace` ends the turn (entry 095); inside one `1`–`9` change an answer,
...
Entry 094's items 6, 9, 10 and 11, as one thing. `../decisions.md` entry 095 is the reasoning and the
...
**What was settled before writing much** — the brief's seven, and entry 095 has the argument for each:
...
- ~~**Whether the action bar should be drawn.**~~ Yes — the second play-through asked for it in as
  many words (entry 094, item 11) and `view/action-bar` built it, entry 095. Whether it is worth the
