...
`docs/decisions.md` entries 058 and 059 are the argument for the last of those.
...
- **Run the matches and write down what they said.** `Commander` drives both sides headless, the
  ...
  entry 059 turned on for the first time in a live behaviour change nothing tested; and
...
entry 058 unblocked it. Suppression, still: nothing depends on it.
