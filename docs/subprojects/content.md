...
`docs/decisions.md` entry 020 and the `<remarks>` on `MapFile`. In one sentence: a map is the
...
**Out of scope.** Loading the map in the sandbox is View's (entry 020 offers it); deleting
