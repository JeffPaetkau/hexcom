...
## 050 — The sandbox fights the mission file, and the fourth copy is gone
...
**For Master.** Row 5 of entry 045's road is done, and row 2 of 049's. Nothing in `game/` names a
map, a soldier or a threshold on the waystation any more; the only hand-placed deployment left is
the compound fixture, which entry 047 made the case for keeping.
