...
public sealed class BattleView
{
    ...
    public void Rebuild(SandboxFrame frame)
    {
        ...
        BuildBodies(frame, bodies, overlay);
    ...
    /// </remarks>
    private static void BuildBodies(SandboxFrame frame, MeshBuilder solid, MeshBuilder overlay)
    {
        var battle = frame.Battle;
        ...
        foreach (var unit in battle.InPlay)
        {
            ...
            var hue = SandboxPalette.SideHue(unit.Side);
            var at = SandboxGeometry.NodePlane(battle.Map, unit.Position);
            var floor = SandboxGeometry.FloorOf(battle.Map, unit.Position);
            var profile = StanceProfile.For(unit.Stance);
            var bearing = unit.Facing.BearingRadians();
            ...
            if (unit == battle.Active)
            {
                var ring = Ring(at, radius + 0.32, floor + Lift * 2, 24);
                overlay.Ribbon(ring, 0.08f, SandboxPalette.TextBright);
            }
        }
    }
    ...
    private void DrawUnitLabels(CanvasItem canvas, SandboxFrame frame)
    {
        ...
        foreach (var unit in battle.InPlay)
        {
            ...
            var hue = SandboxPalette.SideHue(unit.Side);
            var crown = SandboxGeometry.NodeScene(battle.Map, unit.Position)
