    ...
    IReadOnlyList<SandboxDeployment> Deployments)
{
    ...
    /// the same scene to be reachable. The map is loaded by name now rather than built by
    /// <c>DemoMaps.Compound()</c>; entry 024 says the two are identical and a test in
    /// <c>content/</c> holds them so, and going through the library is what lets this file
    /// reference no map-building code at all.
    /// <para>
